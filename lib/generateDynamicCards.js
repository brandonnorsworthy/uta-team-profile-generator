function generateDynamicCards(teamMember) {
    ...
    switch (teamMember.constructor.name) {
        case "Manager":
            ...
            roleSpecificStr = `Office Number: ${teamMember.office}`;
            break;
        case "Engineer":
            //?clickable github username
            //GitHub profile opens in a new tab
            roleIconStr = `smart_toy`;
            roleSpecificStr = `Github: <a href="https://github.com/${teamMember.githubUsername}">@${teamMember.githubUsername}`;
            break;
        case "Intern":
            roleIconStr = `school`;
    ...
    <div class="card">
        ...
        <div class="card-bottom">
            ...
            <p>Email: <a href="mailto:${teamMember.email}?subject=SUBJECTHERE&body=BODYMESSAGE%20STUFF">${teamMember.email}</a></p>
            <p>${roleSpecificStr}</p>
        </div>
    </div>
    `
